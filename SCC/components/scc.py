...
def batched_knn(XA, XB, K, batch_size=1000, offset=0):
    ...
    col = res.flatten()
    d = resd.flatten()
    c = coo_matrix((d[row != col], (row[row != col], col[row != col])), dtype=np.float32,
                   shape=(XB.shape[0], XB.shape[0]))
    return c
...
class TreeLevel(object):
    ...
    def perform_round(self):
        ...
        if row.shape[0] > 1:
            ...
            oneNN_e = time.time()
            logging.debug('Nearest Neighbor: Done. nodes %s, edges %s, time %s', self.dist_graph.shape[0], row.shape[0],
                          oneNN_e - oneNN_s)
            CC_s = time.time()
            num_uniq_parents, parent_map = connected_components(next_round_binary,
                                                                directed=True, connection=self.cc_connection)
            CC_e = time.time()
            logging.debug('Connected Components: Done. nodes %s, edges %s, time %s', next_round_binary.shape[0],
                          next_round_binary.nnz,
                          CC_e - CC_s)
    ...
    def form_next_round(self, next_tau):
        if self.num_uniq_parents > 1:
            ...
            next_counts_nodes = coo_matrix(
                (self.node_counts, (self.parents, np.zeros_like(self.parents, dtype=np.float32))),
        ...
        for round_i, round_obj in tqdm(enumerate(self.rounds)):
            ...
            for j in range(round_obj.parents.shape[0]):
                ...
                if parent_counts[par_j_id] == 1 and round_i > 0 and par_j_id != 'root':  # always write data points
                    ...
                    if j_id in skip_map:
                        j_id = skip_map[j_id]
                    data.append((j_id, par_j_id))
        data = np.array(data)
        tree = np.full(data.shape[0], -1)
        for i in range(len(data)):
            if np.where(data[:, 0] == data[i, 1])[0]:
                tree[i] = np.where(data[:, 0] == data[i, 1])[0]
        if np.where(tree == -1)[0].shape[0] > 1:
            for k in np.where(tree == -1):
                tree[k] = tree.shape[0]
            tree = np.append(tree, -1)
        a1 = np.array(labels)
        if tree.shape[0] > len(labels):
            a2 = np.full(tree.shape[0] - len(labels), 'newRoot')
