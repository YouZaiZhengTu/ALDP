...
class IKMapper():
    ...
    def fit(self, X):
        ...
        assert self._embeding_metrix.shape == (
            n, self._psi * self._t), "Shape of ik_value is incorrect ! Except: %s, Get:%s" % (
        (n, self._psi * self._t), self._embeding_metrix.shape)
    ...
    def transform(self, x):
        ...
        assert ik_value.shape == (
            self._psi * self._t,), "Shape of ik_value is incorrect ! Except: %s, Get:%s" % (
        (self._psi * self._t,), ik_value.shape)
        return ik_value
